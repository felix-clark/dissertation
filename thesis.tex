...
%-------------------------------------------------------------------------------
\documentclass[letterpaper,12pt,oneside,texlive=2018]{memoir} %The memoir class is great for longer works that use separate chapters. The Dissertation Office recommends 10-pt Arial or 12-pt Times New Roman. I use 12-pt for readability.
...
\usepackage{bm}
\usepackage{mathrsfs}
\usepackage[utf8]{inputenc} %% for UTF in bib files
...
%% custom additions, sorted by type
\addbibresource{bibtex/bib/theory.bib}
\addbibresource{bibtex/bib/experiment.bib}
\addbibresource{bibtex/bib/techreport.bib}
\addbibresource{bibtex/bib/images.bib}
...
% default figure width of 0.8 linewidth
...
\usepackage[printonlyused]{acronym} %% options like footnote, nohyperlinks, dua (don't use acronyms)
\usepackage{acronym-defs}
%% use glossaries package instead to include page numbers in the acronym list, or do fancier things

\makeatletter
\g@addto@macro\@floatboxreset\centering
\makeatother

%% capitalize option to always capitalize ``Figure'', ``Equation'', etc.
\usepackage[capitalize]{cleveref}
%% use defaults? will automatically spell out
...
%% we want to number down to sub-sections, however this doesn't add numbers on its own
\setsecnumdepth{subsection} %% \setcounter{secnumdepth} doesn't work for some reason
\setcounter{tocdepth}{3} %% this gives a deeper drop-down navigation depth
...
\hypersetup{pdftitle={ATLAS Thesis},pdfauthor={Michael (Felix) Clark}}
...
%% \chapterstyle{chappell} % Check out the documentation for the memoir class for other options.
%% \chapterstyle{bianchi}
\chapterstyle{madsen}
%% \chapterstyle{veelo} %% a nice style but moves page numbers to the side, which is not allowed per the comment below
...
\newcommand{\thesisTitle}{Femtoscopic signatures of small QGP droplets in proton-lead collisions at the Large Hadron Collider}
\newcommand{\myName}{Felix Michael Clark}
\newcommand{\abstractText}{
  \input{abstract}
}
\newcommand{\yearText}{2019}
...
\begin{center}
  ...
  \vfill %% non-sloppy way to fill with variable space
...
\begin{center}
  \large
  \textbf{ABSTRACT}\\ %You need to keep this text here in all capitals. Don't change it.
  \bigskip
  \textbf{\thesisTitle} \\ %Do of course change this to the title of your dissertation.
  \bigskip
...
%% we can use the \acresetall command in between chapters to reset the acronym memory

\include{Chapter-Introduction/main}       % Introduction
\include{Chapter-Background/main}         % Theory and status of experimental results
\include{Chapter-Experiment/main}         % Description of experimental apparatus
\include{Chapter-Reconstruction/main}     % Reconstruction of charged particles
%% consider dedicated chapter on femtoscopy
\include{Chapter-Analysis/main}           % Description of analysis procedure
\include{Chapter-Results/main}            % Presentation and discussion of results
\include{Chapter-Conclusion/main}         % Concluding remarks
