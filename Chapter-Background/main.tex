...
Dimensional analysis suggests that the critical temperature $T_c$ (at zero chemical potential) should be on the order of magnitude of the only possibly relevant scale
\[
...
% A smooth crossover is observed with a critical temperature of \( T_c = 170 \pm 4 \MeV \) at zero chemical potential \cite{Aoki:2009sc}, which is indeed close to \lqcd as predicted by dimensional analysis.
...
Though Bose-Einstein interactions yield the strongest resolution, in principle any non-trivial interaction can be used to image the source density.
...
When a generalization of the central limit theorem is considered, for which the requirement for finite variance is relaxed, the L\'evy-stable distributions can be derived \cite{Csorgo:2003uv}.
...
\begin{equation}
  ...
  \end{array} \right) + \bigo{ R_{ij,i \neq j}^2 } %%\bigo{ \Ros^2, \Rol^2, \Ros \Rol }
\end{equation}
with $\Rsl = 0$.
...
Collective expansion makes it more difficult for separate regions of the source to equilibrate as they are pushed apart from each other.
...
\begin{figure}[t]
  \includegraphics{phenix_npart_mult.png}
  \caption{Summary of HBT radii as a function of the number of nucleon participants \Npart and of the average multiplicity. The linear scaling of each radius with the cube root of \Npart suggests that the final volume scales linearly with the initial size.}
  \label{fig:hbt_npart_mult}
\end{figure}

Past measurements of the HBT radii exhibit a scaling of each radius with the cube root of \Npart, as shown in \cref{fig:hbt_npart_mult}, although the intercepts are positive and different for each system.
This indicates that the freeze-out size grows with the initial size, because the volume immediately following the collision is proportional to \Npart.
...
\begin{figure}[t]
  \includegraphics{star_azi_hbt_centrality.png}
  \caption{HBT radii in gold-gold collisions as a function of azimuthal angle from the second-order event plane. The source is contracted in-plane and extended out-of-plane, consistent with the expectations from hydrodynamics \cite{Adams:2004wz}.}
  \label{fig:star_azi_hbt}
\end{figure}
...
These measurements show that the transverse radii are reduced along the event plane axis compared to out-of-plane, as shown in \cref{fig:star_azi_hbt}, consistent with the expectations from hydrodynamics.
