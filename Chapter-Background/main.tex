...
Since for identical particles the correlation function is symmetric under exchange, $C(-\mathbf{q}) = C(\mathbf{q})$, A single component of $\mathbf{q}$ can always be chosen as positive.
In this work we will pick the order of a particle pair such that $\qout > 0$.
Aside from the azimuthally-dependent results, the \emph{average} azimuthal symmetry of the \pPb system is invoked so that $C(-\qside) = C(\qside)$, it will be sufficient to consider only the absolute value $|\qside|$ and $\Ros = 0$.
In the azimuthally-dependent results, the \Rol term is sub-dominant compared to the other components\footnote{as shown in the results}.
For those results, only the absolute value of \qlong is considered and \Rol is fixed to zero.
In both cases $\Rls = 0$.
