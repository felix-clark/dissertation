...
Hydrodynamic modeling suggests a formation time on the order of 1 fm/c, so naively the ultra-relativistic initial products of nuclear collisions have the potential to reach thermalization in \AA collisions.
...
Jet quenching has not been observed in \pA collisions, but this does not necessarily rule out the formation of a medium because it could have a formation time comparable to the transverse size of the system.

Jet and particle production in heavy ion collisions is often reported in terms of the nuclear modification factor \RAA.
\begin{equation}
  \RAA = \frac{\frac{d\sigma^{AA}}{d^3 p}}{\langle T_{AA} \rangle \frac{d\sigma^{pp}}{d^3 p}}
\end{equation}
...
The \RAA for $Z$ bosons is shown as a control result because electoweak bosons do not interact via the color force and thus are expected to pass through any \qgp.
...
%% \todo{split this section into \AA and \pA ?} %% maybe split by observable instead

\todo{event plane angle, flow quantities, to discuss before section 5 ``determination of flow vector'' etc. }
\todo{HIGH PRIORITY}

\todo{in \pPb: no jet quenching, hydro results}
In \pp or \pA systems where the projectile has a radius comparable to or smaller than the formation time, hydrodynamics is not expected naively.

general big picture, possible hydro from finger physics: \cite{Busza:2018rrf} %% also nice summary at end
...
, is discussed in greater detail in \cref{sec:hydro}.

%% \subsubsection{Nuclear modification factor} %% probably won't divide into these subsections
...
\todo{is experimental definition of $C(q) = A(q;k)/B(q;k)$ established anywhere?}
...
\begin{figure}[t]
  ...
  \caption{\cite{Lisa:2005dd}.}
  \label{fig:hbt_flow}
\end{figure}
\todo{fill out or merge section, possibly w/ next subsection on motivation}
\cite{Kolb:2003dz} %% decrease w/ kT is collective expansion
