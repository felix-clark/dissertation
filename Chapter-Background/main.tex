...
\begin{equation}
  \beta(\alpha_s) = -\frac{\beta_1  \alpha_s^2}{4\pi}
\end{equation}
where
\[
...
The solution is
\begin{equation}
  \alpha_s(\mu^2) = \frac{4\pi}{\beta_1 \ln\left(\mu^2 / \lqcd^2\right)}
  \label{eq:running_as}
\end{equation}
where \lqcd is defined by a given value for $\alpha_s$ at a scale $\mu_0$
\[
\lqcd = \mu_0 \exp \left( -\frac{2\pi}{\beta_1 \alpha_s(\mu_0^2)} \right) \; .
\]
...
Experimentally it turns out to have a value around
\[
\lqcd \approx 200 \MeV \;,
\]
but the precise value can differ by a factor of 2 or more depending on the exact application and interpretation.
...
The strong coupling constant decreases with rising energy\footnote{In theory, many additional undiscovered quarks at higher energies could eventually cause $\alpha_s$ to increase again if there is ever a point where $N_f > 16$.} as shown in \Cref{eq:running_as} and \Cref{fig:running_coupling}.
This phenomenon is known as \emph{asymptotic freedom} and is necessary to explain Bjorken scaling.

An interesting note is that asymptotic freedom is not present in abelian gauge theories like \ac{QED}; there the screening of a bare electric charge makes the effective charge decrease at large distances, so the effective electric charge increases with higher energy.
