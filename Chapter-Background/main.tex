...
This process is what called \emph{second-order interferometry}, as it requires a decoherent source in contrast to the type of interferometry exhibited in a classic double-slit experiment.
...
Though Bose-Einstein interactions are the easiest to take advantage of, in principle any non-trivial interaction can be used to image the source density.
...
Particles are emitted on a freeze-out hypersurface denoted by $\Sigma$.
...
The hypersurface $\Sigma$ is not a region where each particle is created at, but rather the surface of final interaction of the emitted particles with the bulk.
Though we only integrate $s_n$ over $\Sigma$, the interactions before freeze-out are swept into the definition of $s_n$.
For two particles the following is worked out:
\begin{align}
  C(p_a,p_b) &\equiv \frac{E_a E_b \frac{dN}{d^3 p_a \, d^3 p_b}}{E_a \frac{dN}{d^3 p_a} E_b \frac{dN}{d^3p_b}} 
  \\&= \frac{\iint_{\Sigma} d^3 x_a \, d^3 x_b \, s_2 (p_a, x_a; p_b, x_b) \left| \braket{ p_a, p_b | x_a, x_b } \right|^2}{\int_{\Sigma} d^3 x_a \, s_1 (p_a, x_a) \, \int_{\Sigma} d^3 x_b \, s_1(p_b, x_b)}
\end{align}
...
\begin{equation} S_{2, \mathbf{k}} (\mathbf{r}) \equiv \frac{\iint_{\Sigma} d^3 x_a \, d^3 x_b \, s_2 \left(\frac{m_a}{m_a+m_b}\mathbf{k}, x_a; \frac{m_b}{m_a+m_b}\mathbf{k}, x_b\right) \, \delta^3(\mathbf{r} - \mathbf{x}_a + \mathbf{x}_b)}{\int_{\Sigma} d^3 x_a \, s_1 \left(\frac{m_a}{m_a+m_b}\mathbf{k}, x_a\right) \, \int_{\Sigma} d^3 x_b \, s_1\left(\frac{m_b}{m_a+m_b}\mathbf{k}, x_b\right)}\end{equation}
...
Assuming that the center-of-mass motion is irrelevant after emission, the overall phase $e^{i\mathbf{k}\cdot(\mathbf{x}_a+\mathbf{x}_b)}$ can be innocuously factored out of the wavefunction (as in the example of \Cref{eqn:free_particle_wavefunction}), leaving us only to consider the relative part.
...
The description of the correlation function's enhancement as the Fourier transform of the source density is useful for guiding intuition, but is not sufficient for a modern analysis.
Bowler and Sinyukov pointed out \cite{Bowler:1991vx,Sinyukov:1998fc} that some fraction of pions come from decays or long-lived resonances and that the source function should be modified appropriately.
...
\begin{equation} S_{2,\mathbf{k}}(r) = \lambda S_{c} (r) + (1-\lambda) S_{h} (r) \end{equation}
...
With this simplification the correlation function can be expressed in a fairly general form as a functor of the Fourier transform of the pair source density.
...
In the approximation of ellipsoidal symmetry of the source, the Bose-Einstein part of the identical-pion correlation function can then be expressed as the following:
\begin{equation} C_{\textrm{BE}}(q) = 1 + \mathcal{F}[S_\mathbf{k}](\mathbf{q}) = 1 + e^{- \left|R \mathbf{q} \right|^{\alpha} } \end{equation}
Here $R$ is a symmetric matrix whose components are the \emph{HBT radii}.
...
If $\alpha = 2$, the correlation function is a Gaussian, and so is the source density.
If $\alpha = 1$, the correlation function is a generalized exponential, and the source density is Cauchy.
...
\todo{discuss coordinate system, out-side-long, R matrix}
