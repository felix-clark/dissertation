...
In experiments, impact parameter cannot be measured directly so another proxy is used, typically the charged-particle multiplicity or the total energy deposited in the forward regions.
...
It parameterizes the nuclear density $\rho$ by the Woods-Saxon distribution
\begin{equation}
\rho(r) = \frac{\rho_0}{1 + \exp\left( \frac{r-a}{R} \right)} \, ,
\end{equation}
where $R$ and $a$ are the radius and skin depth of the nucleus, respectively.
The normalization constant $\rho_0$ is set to fix the integral to the total number of nucleons.
...
\begin{figure}[t]
  ...
  \caption{The density as a function of radius following the Woods-Saxon distribution for ${}^{208}\mathrm{Pb}$.}
...
The physics of \pA collisions also turned out to be surprisingly interesting in its own right.


\todo{geometry \ac{GGCF}}
...
\todo{split this section into \AA and \pA ?}
